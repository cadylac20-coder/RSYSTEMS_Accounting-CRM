...
- **`libsql-experimental` is, as the name says, experimental.** I built
  `database.py` so all the SQL in `main.py` is driver-agnostic (it never
  talks to sqlite3 or libsql directly, only through a thin wrapper), but I
  wasn't able to run this against a live Turso database from this sandbox
  (no outbound network access here) — please smoke-test signup/login and a
  few writes against your real Turso DB before relying on it, in case that
  package's API has shifted since this was written.
