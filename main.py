...
def _clamp_day(year: int, month: int, day: int) -> int:
    """Clamp a due-day to the actual number of days in that month, instead
    of always clamping to 28 — a template with due_day=31 (e.g. an ITR
    deadline on the 31st) was previously misdated to the 28th every time,
    even in 31-day months. Only shorter months (Feb, or 30-day months when
    due_day=31) still get clamped, and only as far as that month requires."""
...
def _period_label_and_due(freq: str, due_day: int, due_month_offset: int, ref: datetime):
    ...
    if freq == "monthly":
        ...
        due_date = f"{due_year:04d}-{due_month:02d}-{_clamp_day(due_year, due_month, due_day):02d}"
...
def export_checklists_pdf(data: ExportChecklistsRequest, admin=Depends(require_admin)):
    ...
    def _preserve_typed_formatting(text: str) -> str:
        """
        Keep exact spacing/indentation from what was typed, instead of
        reportlab's default of collapsing whitespace within a line. Each
        line is already rendered as its own flowable (line breaks between
        items/lines are preserved that way already) — this additionally
        preserves leading indentation and multi-space alignment *within*
        a line, so a manually indented sub-point doesn't get flattened.
        """
        ...
        body = _re.sub(r' {2,}', lambda m: "&nbsp;" * len(m.group()), stripped)
        return leading_html + body
    ...
    doc_item_style = ParagraphStyle("Item2", parent=styles["Normal"], fontSize=10.5, leading=16, spaceAfter=4, alignment=TA_LEFT)
    freeform_style = ParagraphStyle("Free2", parent=styles["Normal"], fontSize=10.5, leading=15, spaceAfter=6, alignment=TA_LEFT)
    ...
    for row in rows:
        ...
        header_table = Table([
            ["Service:", _esc(cl["service_type"]), "Checklist:", _esc(cl["name"])],
        ...
        if cl.get("format_mode") == "freeform" and (cl.get("free_text") or "").strip():
            for line in cl["free_text"].split("\n"):
                story.append(Spacer(1, 8) if line.strip() == "" else Paragraph(_preserve_typed_formatting(line), freeform_style))
        else:
            for idx, item in enumerate(documents, 1):
                story.append(Paragraph(f"{idx}. [ ] {_preserve_typed_formatting(str(item))}", doc_item_style))
    ...
    for lrow in letter_rows:
        ...
        if lt.get("subject"):
            story.append(Paragraph(_preserve_typed_formatting(lt["subject"]), section_style))
